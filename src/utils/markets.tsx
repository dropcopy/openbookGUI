...
import MARKETS from '../markets/markets.json';
import TOKEN_MINTS_DATA from '../markets/token-mints.json';
...
export const USE_MARKETS: MarketInfo[] = _IGNORE_DEPRECATED
  ...
  : MARKETS.map((m) => ({ ...m,
                        ...
                        programId: new PublicKey(m.programId)}))
;
const TOKEN_MINTS = TOKEN_MINTS_DATA.map((t)=>({...t, address: new PublicKey(t.address)}));
...
export function getTradePageUrl(marketAddress?: string) {
  if (!marketAddress) {
    ...
    marketAddress = marketAddress || DEFAULT_MARKET?.address.toBase58() || 'srmqPvymJeFKQ4zGQed1GFppgkRHL9kaELCbyksJtPX';
  }
...
export function useMarkPrice() {
  ...
  //kinda nuts but no time to fix.
  //const orderbook = useSelector((state: RootState) => state.openBookbook);
